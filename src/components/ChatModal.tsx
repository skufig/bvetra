    ...
    <div className="fixed inset-0 z-[9999] flex items-end md:items-center justify-center">
      ...
      <div className="relative w-full max-w-lg mx-4 bg-white dark:bg-[#061018] rounded-t-xl md:rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="p-3 border-t flex gap-2 items-center">
          <input value={input} onChange={(e)=>setInput(e.target.value)} onKeyDown={(e)=>{ if(e.key==='Enter'){ sendMessage(input) } }} placeholder={isEn ? 'Ask about services or create a booking...' : 'Спросите о сервисах или оформите бронь...'} className="flex-1 px-3 py-2 rounded border" />
        ...
        <div className="p-3 border-t bg-gray-50 flex gap-2">
          <button onClick={()=>{ setInput(isEn ? 'Create booking' : 'Оформить бронь'); sendMessage(isEn ? 'Create booking' : 'Оформить бронь') }} className="px-3 py-2 rounded border text-sm">{isEn ? 'Create booking' : 'Оформить бронь'}</button>
          <button onClick={()=>{ setInput(isEn ? 'Show fleet' : 'Показать автопарк'); sendMessage(isEn ? 'Show fleet' : 'Показать автопарк') }} className="px-3 py-2 rounded border text-sm">{isEn ? 'Fleet' : 'Автопарк'}</button>
        </div>
      </div>
    </div>
  )
